...
from ClusterQueueOrderSketchParams import *
...
for kind in range(len(workloadName)):
    for prio_num_ in prio_num_arr:
        for i in range(len(load_arr)):
            ...
            directory_name = 'ClusterQueueOrderSketch_%s_%d_%.2d%.2d' % (
                workloadName[kind], int(load_arr[i]*100),
